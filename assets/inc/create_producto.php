<?php
	...
	//validar imagen
	$ruta = "";
	$nom_img = $_FILES["prod_imagen"]["tmp_name"];
	list($ancho, $alto) = getimagesize($nom_img);
	$nuevoancho = 500;
	$nuevoalto = 500;
	$dir = "/assets/images/productos/".$_POST["prod_nombre_comercial"];
	if (isset($dir)) {
		$aleatorio = mt_rand(1000,9999);
		$ruta = "/assets/images/productos/".$_POST["prod_nombre_comercial"]."/".$aleatorio.".png";
		$origen = imagecreatefrompng($_FILES["prod_imagen"]["tmp_name"]);
		$destino = imagecreatetruecolor($nuevoancho, $nuevoalto);
		imagecopyresampled($destino,$origen,0,0,0,0,$nuevoancho,$nuevoalto,$ancho,$alto);
		imagepng($destino,$ruta);
	}else{
		$directorio = "/assets/images/productos/".$_POST["prod_nombre_comercial"];
	...
	$com = intval($_POST['prod_precio_compra']); //compra
	$ven = intval($_POST['prod_precio_venta']); //producto
	$uni = intval($_POST['prod_precio_unitario']); //compra producto
